...
import FetchMock from '../fetch-mock/index.js';
...
export default class ApiService {
  ...
  register(userCredentials) {
    return fetch(API_PATH + '/register', {
      method: 'POST',
      body: JSON.stringify(userCredentials),
    })
      .then(async (response) => {
   ...
   */
  getFiles(folderId) {
    return fetch(API_PATH + `/folder/${folderId}/content`, {
      headers: {
        Authentication: localStorage.getItem('token'),
      },
    })
   ...
   */
  getFolder(id) {
    return fetch(API_PATH + `/folder/${id}`, {
      headers: {
        Authentication: localStorage.getItem('token'),
      },
    })
   ...
   */
  updateFolder(folder) {
    return fetch(API_PATH + `/folder/${folder.id}`, {
      method: 'PUT',
      headers: {
        Authentication: localStorage.getItem('token'),
      },
   ...
   */
  updateFile(file) {
    return fetch(API_PATH + `/file/${file.id}`, {
      method: 'PUT',
      headers: {
        Authentication: localStorage.getItem('token'),
      },
  ...
  uploadFile(folderId, formData) {
    return fetch(API_PATH + `/folder/${folderId}/file`, {
      method: 'POST',
      headers: {
        Authentication: localStorage.getItem('token'),
      },
   ...
   */
  createFolder(id) {
    return fetch(API_PATH + `/folder/${id}/folder`, {
      method: 'POST',
      headers: {
        Authentication: localStorage.getItem('token'),
      },
