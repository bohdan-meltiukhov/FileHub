  ...
  const stateManagerMock = {
    mutate: (mutator) => {
      ...
      if (mutator instanceof LocationMutator) {
        assert.strictEqual(mutator._location, staticPart, 'The HashChangedAction should provide correct location to ' +
          'the state manager.');
      } else if (mutator instanceof LocationParametersMutator) {
        assert.strictEqual(mutator._parameters, dynamicPart, 'The HashChangedAction should provide correct location ' +
          'parameters to the state manager.');
      }
    },
  };
